...
document.getElementById("updateForm").addEventListener("submit", function (event) {
  ...
  fetch("http://localhost:8080/Group_Project_48/backend/api/controllers/adminController.php?action=update_class", requestOptions)
    .then(response => {
...
function openDeletePopup(class_id) {
  ...
  document.getElementById("confirmDelete").onclick = () => {
    ...
    fetch(`http://localhost:8080/Group_Project_48/backend/api/controllers/adminController.php?action=delete_class&class_id=${class_id}`, requestOptions)
      .then(response => {
        return response.json().then(data => {
          if (data.error && data.error === "Token expired") {
            throw new Error("Token expired");
...
function fetchClasses() {
  ...
  fetch("http://localhost:8080/Group_Project_48/backend/api/controllers/adminController.php?action=get_classes", requestOptions)
    .then(response => {
      return response.json().then(data => {
        if (data.error && data.error === "Token expired") {
          throw new Error("Token expired");
